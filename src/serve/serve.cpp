...
ServeApp::ServeApp(const ServeConfig& config, InferenceBackend& backend,
    ...
    // 初始化 SpeakerService
    if (speaker_encoder_) {
        speaker_service_.init(speaker_encoder_.get(), &gpu_mel_);
    }

    // 初始化 TranscriptionPipeline (组件通过 Dependencies 注入)
    if (asr_plugin_) {
        asr::TranscriptionPipeline::Dependencies deps;
        deps.asr_plugin = asr_plugin_.get();
        deps.speaker_encoder = speaker_encoder_.get();
        deps.speaker_manager = &speaker_manager_;
        deps.speaker_mutex = &speaker_mutex_;
        deps.vad_engine = &vad_engine_;
        deps.gpu_vad_engine = &gpu_vad_engine_;
        deps.vad_mutex = &vad_mutex_;
        deps.gpu_mel = &gpu_mel_;
...
void ServeApp::handle_audio_transcriptions(const HttpRequest& req, int client_fd) {
    ...
    // 提取参数
    asr::TranscriptionParams params;
    params.language = form.fields.count("language") ? form.fields["language"] : "auto";
    std::string response_format = form.fields.count("response_format") ?
                                  form.fields["response_format"] : "json";
    params.suppress_early_eos = form.fields.count("suppress_early_eos") &&
                              (form.fields["suppress_early_eos"] == "true" ||
                               form.fields["suppress_early_eos"] == "1");
    params.punctuate = form.fields.count("punctuate") &&
                     (form.fields["punctuate"] == "true" || form.fields["punctuate"] == "1");
    params.identify_speaker = form.fields.count("speaker") &&
                        (form.fields["speaker"] == "true" || form.fields["speaker"] == "1");
    params.clean_oral = form.fields.count("clean") &&
    ...
    fprintf(stderr, "[Serve] ASR: received %zu bytes audio (%s), language=%s%s%s%s%s\n",
            audio_data.size(), audio_filename.c_str(), params.language.c_str(),
            params.suppress_early_eos ? ", suppress_early_eos" : "",
            params.punctuate ? ", punctuate" : "",
            params.identify_speaker ? ", speaker_id" : "",
            params.want_word_timestamps ? ", word_timestamps" : "");
    ...
    // 委托给 TranscriptionPipeline
    auto result = transcription_pipeline_->transcribe(wav, params);
    ...
    // 格式化响应
    HttpResponse resp;
    float total_duration_s = result.duration_s;
    ...
    if (response_format == "text") {
        ...
        if (!result.segments.empty()) {
            ...
            resp.body = !result.full_text_with_punc.empty() ?
                        result.full_text_with_punc : result.full_text;
        }
    } else if (response_format == "verbose_json") {
        std::string out_text = result.full_text;
        resp.body = "{\"task\":\"transcribe\",\"language\":\"" + json_escape(params.language) +
                    "\",\"duration\":" + std::to_string(total_duration_s) +
                    ",\"text\":\"" + json_escape(out_text) + "\"";
        if (!result.full_text_with_punc.empty())
            resp.body += ",\"text_with_punc\":\"" + json_escape(result.full_text_with_punc) + "\"";
        // words
        if (!result.words.empty()) {
            ...
            for (size_t i = 0; i < result.words.size(); ++i) {
                ...
                if (!w.speaker_name.empty() && w.speaker_id >= 0)
                    resp.body += ",\"speaker\":\"" + json_escape(w.speaker_name) + "\"";
                resp.body += "}";
            }
            resp.body += "]";
        }
        ...
        if (!result.segments.empty()) {
            ...
            for (size_t i = 0; i < result.segments.size(); ++i) {
                auto& s = result.segments[i];
                if (i > 0) resp.body += ",";
                resp.body += "{\"start\":" + std::to_string(s.start_ms / 1000.0f) +
                             ",\"end\":" + std::to_string(s.end_ms / 1000.0f) +
                             ",\"text\":\"" + json_escape(s.text) + "\"";
                if (s.speaker_id >= 0) {
                    resp.body += ",\"speaker\":\"" + json_escape(s.speaker_name) +
                                 "\",\"speaker_id\":" + std::to_string(s.speaker_id);
                    if (s.speaker_similarity > 0)
                        resp.body += ",\"speaker_similarity\":" + std::to_string(s.speaker_similarity);
                }
                resp.body += "}";
            }
            resp.body += "]";
        ...
        // 默认 json
        std::string out_text = !result.full_text_with_punc.empty() ?
                               result.full_text_with_punc : result.full_text;
...
asr::SpeakerManager::MatchResult ServeApp::identify_speaker(
    ...
    return speaker_service_.identify(samples, num_samples, sample_rate, auto_register);
}
...
void ServeApp::handle_speaker_register(const HttpRequest& req, int client_fd) {
    ...
    int num_frames = 0;
    asr::SpeakerService::compute_mel_80(wav.samples.data(), (int)wav.samples.size(),
                                         wav.sample_rate, mel, num_frames);
...
// ============================================================================
// WebSocket /v1/voice — 委托给 VoiceSession
// ============================================================================
...
void ServeApp::handle_websocket_voice(int client_fd, const HttpRequest& req) {
    ...
    VoiceSession session(this, VoiceSession::ProtocolMode::VOICE, client_fd);
    session.run();
}

// ============================================================================
// WebSocket /v1/realtime — 委托给 VoiceSession
// ============================================================================
...
void ServeApp::handle_websocket_realtime(int client_fd, const HttpRequest& req) {
    ...
    VoiceSession session(this, VoiceSession::ProtocolMode::REALTIME, client_fd);
    session.run();
}
