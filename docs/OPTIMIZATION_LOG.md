...
### A/B 验证: Projection Merge 净效果

**背景**: 历史记录 bench_final.txt (17:14) 显示 223.11ms ITL / 212.66ms Forward / 229.7 GB/s，
而当前代码（含 merge + fusions）稳定在 229ms。用户质疑是否存在性能倒退。

**关键发现**: bench_final.txt 使用 `--decode 5 --warmup 2`（仅 5 个样本），且无 merge。
通过 A/B 对比（相同参数 `--decode 30 --warmup 5 --kv-cache-gb 4`）验证：

| 配置 | ITL (median) | Forward (median) | BW (median) |
|------|-------------|-----------------|-------------|
| 无 merge (3 separate GEMV/层) | 232.81 ms | 221.91 ms | 220.1 GB/s |
| 有 merge (fused RMSNorm+GEMV) | 229.22 ms | 218.29 ms | 223.6 GB/s |
| bench_final 历史值 (N=5) | 223.11 ms | 212.66 ms | 229.7 GB/s |

**结论**:
1. Merge **确实提速 ~3.6ms** (232.8 → 229.2)，净正向收益
2. bench_final 的 223ms 是 N=5 样本不足 + 带宽瞬时波动 (229.7 vs 稳定 220-224 GB/s) 导致的异常值
3. GPU/EMC 频率确认全锁死 (GPC 1575 MHz, EMC 4266 MHz)，排除降频因素
4. bench 必须使用 `--decode 30 --warmup 5` 以上才具统计意义

### 当前性能基线 (2026-03-05)

**B=1 Decode (kv=4GB, decode=30, warmup=5, no-graph)**:
- ITL: 229.2ms median, 4.36 tok/s
- Forward: 218.3ms median
- Weight BW: 223.6 GB/s (82% of 273 GB/s peak)
- Total weight reads: 51.2 GB/step (BF16)

**累计优化效果** (相对初始代码):
- Level 1 SSM BF16化: B=1 无回退, B=128 吞吐 +42.6%
- Level 2 Projection Merge (QKV×16 + QKVZAB×48): -3.6ms/step, 176 launches saved
- QK_norm + RoPE Fusion: 32 launches saved, marginal latency
- RMSNorm + GEMV Fusion: 64 launches saved, ~1ms improvement
- 累计 launch 节省: ~272/step
